...
이 과정에서 인증 실패 상황에서 발생한 Exception이 기존 방식으로 처리되면서 서비스 간 요청에 적절한 오류 응답을 전달하지 못하는 문제가 발생했습니다. 인증 요청부터 Exception 발생, HTTP 응답까지의 흐름을 확인하고 인증 관련 Exception 처리 클래스의 에러 코드를 상세히 작성하여 오류 상황에서 빠른 대처가 가능하도록 개선했습니다.
